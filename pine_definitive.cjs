// pine_definitive.cjs - Ctrl+End -> Shift+Ctrl+Home (select all) -> Delete -> insertText full -> verifikasi
const fs = require('fs');
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    async function viewTop() {
      return await evalJS(`(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ls=ed.querySelectorAll('.view-line');var nums=ed.querySelectorAll('.line-numbers');var out=[];for(var i=0;i<ls.length&&i<4;i++){var n=nums[i]?nums[i].textContent:'?';out.push(n+':'+ls[i].textContent.slice(0,35))}return JSON.stringify(out)})()`);
    }
    ws.on('open', async () => {
      ...
      await sleep(300);
      // 2) Ctrl+End -> kursor akhir dokumen
      await key(2, 'End', 'End', 35); await sleep(600);
      // 3) Shift+Ctrl+Home -> pilih dari akhir ke awal (select all)
      await key(3, 'Home', 'Home', 36); await sleep(600);
      // 4) Delete -> hapus semua
      await key(0, 'Delete', 'Delete', 46); await sleep(800);
      console.log('after delete:', await viewTop());
      // 5) insertText full source
      await send('Input.insertText', { text: src });
      console.log('insert sent, waiting 40s...');
      await sleep(40000);
