// pine_replace_final.cjs - Ctrl+A -> 'X' (bersihkan) -> Ctrl+A (pilih X) -> insertText FULL source
const fs = require('fs');
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    ws.on('open', async () => {
      ...
      await sleep(300);
      // 2) Ctrl+A (select all model lama)
      await key(2, 'a', 'KeyA', 65); await sleep(400);
      // 3) insert 'X' menggantikan seluruh model -> model = X
      await send('Input.insertText', { text: 'X' }); await sleep(400);
      console.log('step1 (X):', await viewTop());
      // 4) Ctrl+A (pilih X)
      await key(2, 'a', 'KeyA', 65); await sleep(300);
      // 5) insert source penuh (menggantikan X)
      await send('Input.insertText', { text: src });
      await sleep(2000);
      console.log('step2 (FULL):', await viewTop());
      // 6) cek baris akhir via Ctrl+End
      await key(2, 'End', 'End', 35); await sleep(800);
      const endView = await evalJS(`(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ls=ed.querySelectorAll('.view-line');var nums=ed.querySelectorAll('.line-numbers');var out=[];for(var i=Math.max(0,ls.length-3);i<ls.length;i++){var n=nums[i]?nums[i].textContent:'?';out.push(n+':'+ls[i].textContent.slice(0,50))}return JSON.stringify(out)})()`);
      console.log('END view:', endView);
      ws.close();
      process.exit(0);
    });
  });
