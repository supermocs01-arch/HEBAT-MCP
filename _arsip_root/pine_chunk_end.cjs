// pine_chunk_end.cjs - insert chunk + key End di antara (kursor ke akhir dokumen)
const fs = require('fs');
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    async function getLen() {
      return await evalJS(`(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ia=ed.querySelector('.inputarea');return (ia.value||'').length})()`);
    }
    ws.on('open', async () => {
      ...
      await send('Input.insertText', { text: 'X' }); await sleep(300);
      await key(2, 'a', 'KeyA', 65); await sleep(300);
      await key(0, 'Backspace', 'Backspace', 8); await sleep(400);
      console.log('kosong, len:', await getLen());
      const CH = 380;
      for (let i = 0; i < src.length; i += CH) {
        await send('Input.insertText', { text: src.slice(i, i + CH) });
        await sleep(500);
        await key(0, 'End', 'End', 35); // kursor ke akhir baris/dokumen
        await sleep(300);
        if (i % (CH*3) === 0) console.log('  progress', Math.min(i+CH, src.length));
      }
      await sleep(1000);
      console.log('FINAL len:', await getLen(), '| expected', src.length);
      ws.close();
      process.exit(0);
    });
  });
