// pine_total_model.cjs - hitung total baris model via scroll ke bawah + baca baris akhir
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    async function readViewport() {
      return await evalJS(`(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ls=ed.querySelectorAll('.view-line');var nums=ed.querySelectorAll('.line-numbers');var out=[];for(var i=0;i<ls.length;i++){var n=nums[i]?nums[i].textContent:'?';out.push(n+':'+ls[i].textContent.slice(0,40))}return JSON.stringify(out)})()`);
    }
    ws.on('open', async () => {
      ...
      console.log('== bottom viewport ==');
      const v = JSON.parse(await readViewport());
