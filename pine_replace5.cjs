// pine_replace5.cjs - Ctrl+A -> ketik X (replace all) -> verifikasi 1 char -> insertText penuh
const fs = require('fs');
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    ws.on('open', async () => {
      ...
      await sleep(400);
      // 3) ketik 'X' via insertText (mengganti seleksi)
      await send('Input.insertText', { text: 'X' });
      await sleep(500);
      // 4) verifikasi ia.value = 1
      const r1 = await send('Runtime.evaluate', {
        expression: `(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ia=ed.querySelector('.inputarea');return JSON.stringify({len:ia.value.length,val:ia.value})})()`,
        returnByValue: true
      });
      console.log('after X:', r1.result && r1.result.value);
      // 5) Ctrl+A (pilih X)
      await key(2, 'a', 'KeyA', 65);
      await sleep(300);
      // 6) insertText source penuh
      await send('Input.insertText', { text: src });
      await sleep(1500);
      const r2 = await send('Runtime.evaluate', {
        expression: `(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ia=ed.querySelector('.inputarea');var v=ia.value||'';return JSON.stringify({len:v.length,head:v.slice(0,50),nl:(v.match(/\\n/g)||[]).length})})()`,
        returnByValue: true
      });
      console.log('final:', r2.result && r2.result.value);
      ws.close();
      process.exit(0);
    });
  });
