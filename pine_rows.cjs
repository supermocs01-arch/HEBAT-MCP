// pine_rows.cjs - PageDown bertahap baca semua baris via key
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    async function readRows() {
      return await evalJS(`(()=>{var ed=document.querySelectorAll('.monaco-editor.pine-editor-monaco')[1];var ls=ed.querySelectorAll('.view-line');var nums=ed.querySelectorAll('.line-numbers');var out=[];for(var i=0;i<ls.length;i++){out.push((nums[i]?nums[i].textContent:'?')+' :: '+JSON.stringify(ls[i].textContent.slice(0,60)))}return JSON.stringify(out)})()`);
    }
    ws.on('open', async () => {
      ...
      // baca halaman 1 (baris atas)
      let page = await readRows();
      page = JSON.parse(page);
      console.log('== top ==');
      page.filter(x => { const n = parseInt(x.split(':')[0],10); return !isNaN(n) && n <= 40; }).forEach(x => console.log(x));
      process.exit(0);
    });
  });
