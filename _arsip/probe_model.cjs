      ...
      const r = await send('Runtime.evaluate', {
        expression: `(function(){
          try {
            ...
            var m = cw.model().m_model;
            var keys = Object.keys(m);
            var found = [];
            keys.forEach(function(k){
              if (/chart|override|propert/i.test(k)) found.push(k);
            });
            var mKeys = m.mainSeries ? Object.keys(m.mainSeries()) : [];
            return JSON.stringify({ chartKeys: found, mKeys: mKeys.slice(0,60) });
