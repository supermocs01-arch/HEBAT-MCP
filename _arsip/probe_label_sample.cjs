      ...
      const r = await send('Runtime.evaluate', {
        expression: `(function(){
          try {
            ...
            var col = g._primitivesCollection.dwglabels.get('labels').get(false)._primitivesDataById;
            var out = { total: col.size };
            col.forEach(function(l){
              out.keys = Object.keys(l);
              out.sample = l;
              return true;
            });
            var out2 = JSON.parse(JSON.stringify(out));
