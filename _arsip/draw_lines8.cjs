...
http.get('http://127.0.0.1:9222/json', (res) => {
  ...
  res.on('end', () => {
    ...
    ws.on('open', async () => {
      ...
      // First explore available methods on the widget
  ...
  try {
    ...
    // Try the createShape method with full params
    ...
    levels.forEach(function(l) {
      try {
        var r = wv.createShape('horizontal_line', {
          points: [{time: time, price: l.price}, {time: time + 86400, price: l.price}],
          text: l.text,
          color: l.color,
          linewidth: 2,
          lock: true,
        ...
        results.push({price: l.price, ok: !!r, type: typeof r});
      } catch(e) {
        results.push({price: l.price, err: e.message.substring(0,100)});
      }
    });

    return JSON.stringify({createShape: results});
