// pine_chunk_probe.cjs - insert chunk 400, verifikasi len setelah tiap chunk
const fs = require('fs');
...
http.get('http://127.0.0.1:9222/json', res => {
  ...
  res.on('end', () => {
    ...
    ws.on('open', async () => {
      ...
      await sleep(300);
      // kosongkan via Ctrl+A + X + backspace
      ...
      // kirim chunk 400
      for (let i = 0; i < src.length && i < 2000; i += 400) {
        await send('Input.insertText', { text: src.slice(i, i + 400) });
        await sleep(700);
        console.log('chunk', i, '-> len:', await getLen());
      }
      ws.close();
      process.exit(0);
    });
